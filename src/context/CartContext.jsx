...
const CartProvider = ({children}) => {
    ...
    const totalPrice = () => {
        const price = cart.reduce((total, productCard) => total + (productCard.quantity * productCard.precio), 0)
        return price
    }

    const deleteProductById = (idProduct)=> {
        const filterProducts = cart.filter((productCard)=> productCard.id !== idProduct)
